...
export async function handleNotification(req: Request): Promise<Response> {
  try {
    ...
    const webhookUrl = Deno.env.get("CLINIC_NOTIFICATION");
    if (!webhookUrl) {
      console.error("Missing CLINIC_NOTIFICATION webhook URL");
      throw new Error("Webhook URL not configured");
    }

    // Parse the notification payload from the request
    const payload: NotificationPayload = await req.json();
    console.log("Received clinic notification payload:", JSON.stringify(payload));
    ...
    console.error("Error processing clinic notification:", error);
    return new Response(
      JSON.stringify({
        success: false,
...
async function formatPayloadForGHL(
  ...
  try {
    ...
    if (payload.notification_type === "payment_success") {
      ...
      return await formatPaymentSuccess(payload.record_id, supabaseClient);
