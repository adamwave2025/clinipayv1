...
const RefundDialog: React.FC<RefundDialogProps> = ({
  ...
  const [refundAmountDisplay, setRefundAmountDisplay] = React.useState<string>('');
  const [isPartialRefund, setIsPartialRefund] = React.useState<boolean>(false);
  const [hasError, setHasError] = React.useState<boolean>(false);

  // Reset form state when dialog opens
  React.useEffect(() => {
    if (open) {
      // Format the initial amount when dialog opens (convert pence to pounds for display)
      const formattedAmount = penceToPounds(paymentAmount || 0).toFixed(2);
      setRefundAmountDisplay(formattedAmount);
      setRefundAmount(penceToPounds(paymentAmount || 0));
  ...
  const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (/^[0-9]*\.?[0-9]{0,2}$/.test(inputValue) || inputValue === '') {
      ...
      const maxRefundInPounds = penceToPounds(paymentAmount || 0);
  ...
  const handleProcessRefund = () => {
    if (isPartialRefund && refundAmount !== null && refundAmount > 0) {
      // Partial refund with specified amount (in pounds)
      onRefund(refundAmount);
    } else {
      // Full refund (provide the full amount in pounds)
      onRefund(penceToPounds(paymentAmount || 0));
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Process a refund for this payment of {formatCurrency(paymentAmount || 0)}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="grid gap-2">
            <div className="flex items-center">
              <input
                ...
                className="mr-2"
              />
              <Label htmlFor="fullRefund">Full refund ({formatCurrency(paymentAmount || 0)})</Label>
            </div>
              ...
              <div className="ml-6 mt-2">
                ...
                {hasError && (
                  <p className="text-xs text-red-500 mt-1">
                    Refund amount must be between £0.01 and {formatCurrency(paymentAmount || 0)}.
                  </p>
                )}
              </div>
            )}
